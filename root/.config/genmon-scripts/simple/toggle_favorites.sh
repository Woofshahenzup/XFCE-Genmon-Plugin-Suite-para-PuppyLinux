...
FILES=("hdsentinel" "fusilli" "favorites" "btc" "cpu-temp" "keyboard")

# Asegurar que el directorio existe
mkdir -p "$HOME/.config/genmon-hide"

# Archivo que mantiene el estado global del toggle
TOGGLE_STATE_FILE="$HOME/.config/genmon-hide/.toggle_state_favorites"
# === 🖋️ Configuración tipográfica ===
...
# Determinar el estado general basado en "hdsentinel"
if [ -f "$HOME/.config/genmon-hide/hdsentinel" ]; then
...
ACTION="<txtclick>/root/.config/genmon-scripts/simple/toggle_favorites.sh toggle</txtclick>"

# Tooltip estilizado con fuente personalizada
